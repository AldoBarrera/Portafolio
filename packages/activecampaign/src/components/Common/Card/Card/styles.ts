...
export const Card = styled.div.attrs({
  ...
  ${({
    ...
    display: flex;
    flex-direction: ${
      isHorizontal ? (isHorizontalInverted ? 'row-reverse' : 'row') : 'column'
    };
    min-height: 0;
    ...
    border-top-left-radius: ${
      borderTopLeftRadius ? `${theme.spaces.large}` : 0
    }; 
    border-top-right-radius: ${
      borderTopRightRadius ? `${theme.spaces.large}` : 0
    };
    border-bottom-right-radius: ${
      borderBottomRightRadius ? `${theme.spaces.large}` : 0
    };
    border-bottom-left-radius: ${
      borderBottomLeftRadius ? `${theme.spaces.large}` : 0
    };
    ${
      isHorizontal &&
      css`
