...
import { useMediaQuery } from 'components/Common/Button/hooks/useMediaQuery'
...
const renderCard = (items, disableLinks = true, size = null) => {
  return items?.map((item, index) => (
    <Grid.Column key={index} size={size ?? item.gridSize}>
      <Link
        ...
        prefetch={false}
      >
        <Card key={index} size="auto" hasBorder={false}>
          {item.icon && (
          ...
          <Card.Content padding="30px">
            <Card.Header
              textAlign={item.configuration?.textAlign}
              textProperties={item.configuration?.headerProperties}
              content={item.title}
            />
            <Card.Description
              textAlign={item.configuration?.textAlign}
              textProperties={item.configuration?.descriptionProperties}
...
function SliderCardsSize({
  ...
  const isMobile = useMediaQuery(`(max-width: ${defaultBreakpoints.medium})`)
  return (
