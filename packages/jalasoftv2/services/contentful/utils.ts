...
export const getData = async (page: string, include = 10, item = 0) => {
  try {
    ...
    throw new WebsiteError(
      ...
      `${ERRORS.CONTENTFUL_DATA} '${page}' - ${typedError.message}`
    )
  }
}
...
export function searchBlogsFromClient({
...
export async function getGlobalConfig() {
  const ContentfulData = await client.getEntries({
    content_type: PAGES.GLOBAL_CONFIG.data,
    include: 4
  })
  return dataParseContentful(ContentfulData.items[0])
}
