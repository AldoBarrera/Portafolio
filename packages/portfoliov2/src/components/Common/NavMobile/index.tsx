...
function NavMobile({ iconColor, topLinks, mainLinks }: NavPropsMobile) {
  ...
  const toggleVisibility = (link: NavType) => {
    setShowNavSubMenu({
      ...showNavSubMenu,
      [link.label]: !showNavSubMenu[link.label as keyof typeof showNavSubMenu]
    })
  }

  const renderArrowIcon = (link: NavType) => {
    if (link.navItems) {
      if (showNavSubMenu[link.label as keyof typeof showNavSubMenu]) {
            ...
            {mainLinks &&
              mainLinks.map((link) => (
                ...
                >
                  ...
                  <ToggleVisibility open={showNavSubMenu[link.label as keyof typeof showNavSubMenu]}>
                    <Style.NavSubMenu>
